...
```text
fccba73 (HEAD -> framework, origin/framework) Record mixed-audio dry-run status
...
```text
 ...
 eval/streaming_sst/eval_mixed_audio_switch.py | 662 ++++++++++++++++++++++++++
 test_mixed_audio_switch_eval.py               | 136 ++++++
...
Current file excerpt/full content, possibly truncated:
```text
Verification before Claude review:

Git:

```text
Branch: framework
HEAD: fccba73 Record mixed-audio dry-run status
Remote: git@github.com:luojiaxuan/rasst-demo.git
origin/framework: fccba73
Taurus checkout: /mnt/taurus/home/jiaxuanluo/rasst-demo at fccba73
```
...
```text
$ cd /mnt/taurus/home/jiaxuanluo/rasst-demo && python3 -m unittest test_mixed_audio_switch_eval test_mixed_domain_switch_eval test_hybrid_window_topic_router test_auto_glossary_switch_eval
.........................
----------------------------------------------------------------------
...
- blocks:
  1 2022.acl-long.268 nlp 687.2s
  2 medicine_404 medicine 3211.1s
  3 2022.acl-long.367 nlp 612.3s
  4 medicine_606 medicine 2842.8s
  5 2022.acl-long.590 nlp 517.1s
  6 medicine_545006 medicine 1239.1s
  7 2022.acl-long.110 nlp 639.4s
  8 medicine_596001 medicine 3126.7s
  9 2022.acl-long.117 nlp 651.3s
  10 medicine_605000 medicine 3321.1s

random_seed20260707_audio_playlist_dryrun.json:
- block_count=10
- audio_seconds=16848.115
- random seed 20260707 order includes 5 ACL and 5 medicine blocks.
```

Server/GPU status:

```text
Taurus 127.0.0.1:8011 health:
- status healthy, mock_mode=false
- router_mode=embedding_refs
- active term memory snapshot=ai_glossary_sweep_20260706_aries
- active index=acl_ai_broad_plus10k
- therefore invalid for target generated-target/probe router evidence.

Taurus GPU preflight:
- selected_gpu_csv=4 only.
- GPUs 5/6/7 are occupied by current server; GPUs 0-3 by other busy jobs.

Aries check:
- / is 100% full.
- GPUs 0-5 hold about 41-45GB each; GPU7 empty, GPU6 partially used.
- Not enough clean resource to start a separate TP2 Omni server plus RAG safely.
```
