...
AutoTerm-SST treats this as an active-inventory and ranking problem. The automatic mode starts from a domain-specific initial slice and routes directly among domain slices when streaming evidence is strong. A training-free router combines generated target-translation topic windows, speech-window domain probes, speech-centroid evidence, and retrieved-reference metadata; broad open memory is used only as a fallback candidate pool under low confidence. Candidates from the active inventory are reranked into a capped top-10 retrieval list before score filtering.
...
\begin{itemize}
    ...
    \item We introduce a zero-setup adaptive terminology strategy that retrieves up to 10 candidates from domain-specific active slices and low-confidence fallback pools, then filters low-confidence matches before prompting.
    \item We provide system evidence covering terminology recall on a smoke talk, prompt-reference volume, retrieval latency, scale up to million-term memories, mixed-domain routing behavior, and concurrent streaming behavior.
\end{itemize}
