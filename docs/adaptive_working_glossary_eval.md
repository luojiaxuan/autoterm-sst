...
| in progress | Medicine slice quality fix | Union + indexes + manifest done 2026-07-08: `medicine_hardraw_gt_union_gs10000.json` (212 GT byte-identical + 9,788 `wiki_medicine_zh` fillers, 606 oracle 54/54; HF `204ba141`/`365c84ba`), MaxSim indexes for `medicine_hardraw_gs10k` + eval-only `medicine_hardraw_oracle` built on aries GPU5, registered in `manifests/auto_working_medicine_hardraw_20260708.json` (loader-validated, `current.json` untouched). Truncated 3-talk rerun done 2026-07-09 on aries 8013: auto medicine type_acc 21/23 vs none 9/23, 2/2 switches 0 wrong (35.5s/25.0s), steady-state 1.0; see auto_glossary_mixed_switch_20260707.md. Remaining: 10-talk final run with both domain slices as gs unions (nlp side: acl_tagged_gs10k). |
