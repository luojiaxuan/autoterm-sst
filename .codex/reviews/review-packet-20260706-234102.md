...
```text
22fd011 (HEAD -> framework, origin/framework) Document and test generated-target routing guards
7449b53 Harden generated-target glossary routing
...
```text
 ...
 docs/adaptive_working_glossary.md               | 125 ++++++++++++----------
 ...
 test_auto_working_fixed_top10.py                | 133 ++++++++++++++++++++++--
 test_hybrid_window_topic_router.py              | 133 +++++++++++++++++++++++-
 11 files changed, 453 insertions(+), 118 deletions(-)
...
index 2ebb1ca..65e9872 100644
--- a/README.md
+++ b/README.md
@@ -250,16 +250,18 @@ retrieves from it exactly like a glossary, with retrieved terms surfaced in the
...
+router_text_source          # generated_target / manifest_source / streaming_asr / none
 ...
 last_topic_update_s
 topic_history
@@ -76,12 +77,19 @@ active preset is `RASST_AUTO_GLOSSARY_DEFAULT` (`nlp_core_10k` by default).
...
@@ -110,10 +118,11 @@ top-k probe over ready candidate domain indexes; fresh probe runs are gated by
...
@@ -132,15 +141,17 @@ The earlier `embedding_refs` implementation is not sufficient for robust cross-d
...
@@ -256,7 +267,7 @@ The JSON WebSocket event contains:
       "margin": 0.19,
...
@@ -277,7 +288,7 @@ router. The current production defaults are:
 ```yaml
 auto_working:
   prompt_k: 10
-  base_slice: common_terms
...
@@ -359,14 +374,15 @@ and the clean fixture regression with contested synthetic probe evidence passed
...
@@ -422,11 +438,10 @@ evidence, but the demo should select and rank candidates from an active
...
@@ -434,12 +449,14 @@ inventory. The claim is:
...
index 3e250b6..78cd944 100644
--- a/docs/adaptive_working_glossary_eval.md
+++ b/docs/adaptive_working_glossary_eval.md
@@ -78,9 +78,12 @@ the renormalized probe score can switch domains.
...
+from framework.agents.plugins.retrieval import MockRetrieval, RetrievalResult
 from framework.agents.term_memory.slice_registry import (
     ...
     rank_references,
@@ -36,15 +38,19 @@ class AutoWorkingFixedTop10Tests(unittest.TestCase):
...
@@ -109,6 +115,117 @@ class AutoWorkingFixedTop10Tests(unittest.TestCase):
...
+
     def test_domain_probe_slices_are_domain_only_debug_inventory(self) -> None:
         agent = OmniAgent()
         agent.config.mock = True
@@ -128,7 +245,7 @@ class AutoWorkingFixedTop10Tests(unittest.TestCase):
...
+    def test_auto_active_retrieval_uses_domain_specific_slice_only(self) -> None:
         ...
         agent.config.mock = True
         session = SimpleNamespace(
@@ -144,9 +261,9 @@ class AutoWorkingFixedTop10Tests(unittest.TestCase):
...
+        self.assertEqual(session.active_slice_presets, ["nlp_core_10k"])
     ...
     def test_domain_probe_populates_metadata_without_changing_active_inventory(self) -> None:
         agent = OmniAgent()
@@ -172,7 +289,7 @@ class AutoWorkingFixedTop10Tests(unittest.TestCase):
...
@@ -263,7 +380,7 @@ class AutoWorkingFixedTop10Tests(unittest.TestCase):
         )
...
@@ -84,6 +112,107 @@ class HybridWindowTopicRouterTests(unittest.TestCase):
...
+
     def test_audio_only_probe_requires_three_consistent_windows(self) -> None:
         router = _router()
         state = RouterSessionState("nlp_core_10k", "nlp", created_s=1.0)
@@ -164,14 +293,14 @@ class HybridWindowTopicRouterTests(unittest.TestCase):
...
After second accepted Claude pass:
