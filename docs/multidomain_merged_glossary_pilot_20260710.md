...
按固定 5 ACL + 5 Medicine alternating playlist 的 full run 已于
2026-07-10 09:45:20 UTC 启动；正式结论以 full 10-talk MFA 结果为准。
...
3-talk 达到最小正向 signal，但 4-talk 没有复现，因此当前不能说 AutoTerm 在
固定 40k universe 上优于 zero-session-setup merged baseline。在线切换方向稳定，
但 quality claim 必须等待正在运行的 full 10-talk。后续仍需：
...
- 4-talk replication 与正在运行的 full 10-talk：
