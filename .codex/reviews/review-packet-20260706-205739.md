...
- Branch: framework
- HEAD: df1401b
- Requested base branch: 8a911aa
- Effective base branch: 8a911aa
- Merge base: 8a911aa6f37551517ed63e8fcf62b8ef8db0a6b7
- Base warning: [none]
...
```text
df1401b (HEAD -> framework, origin/framework) Guard audio-only domain probe switches
...
```text
 configs/autoterm_slices.yaml                    |  3 ++
 docs/adaptive_working_glossary.md               | 22 ++++++--
 docs/adaptive_working_glossary_eval.md          | 13 +++--
 docs/auto_glossary_routing_probe_20260707.md    | 16 +++---
 eval/streaming_sst/eval_auto_glossary_switch.py | 11 +++-
 framework/agents/omni.py                        | 13 +++++
 framework/agents/plugins/retrieval.py           |  5 +-
 framework/agents/term_memory/topic_router.py    | 67 +++++++++++++++++++++++++
 test_auto_working_fixed_top10.py                | 10 ++++
 test_hybrid_window_topic_router.py              | 36 +++++++++++++
...
index df83ef3..94c5356 100644
--- a/eval/streaming_sst/eval_auto_glossary_switch.py
...
@@ -627,6 +638,62 @@ class HybridWindowTopicRouter(AudioNativeActiveGlossaryRouter):
...
+            ),
             "medicine": DomainProbeScore(
                 "medicine",
                 "medicine_core_10k",
@@ -109,6 +116,35 @@ class HybridWindowTopicRouterTests(unittest.TestCase):
...
class OmniAgent(Agent):
    ...
    def _vllm_config(self) -> Dict[str, Any]:
        ...
        catalog = self._catalog(self.config
            ...
            for row_idx, (_, meta) in enumerate(group_items):
                ...
                if int(row_valid.sum().item()) > 0:
                    ...
                    if bool(requests[request_idx].get("return_query_window_embeddings")):
                        outputs[request_idx].query_window_embeddings = valid_window_embs.detach().cpu().float()
                    pooled = valid_window_embs.mean(dim=0)
                    pooled = F.normalize(pooled, p=2, dim=-1).detach().cpu().floa
...
class HybridWindowTopicRouter(AudioNativeActiveGlossaryRouter):
    ...
    def _score_hybrid(
        ...
        *,
        ...
        has_text = bool((router_text or "").strip() and str(router_text_source or "none") != "n
...
Claude review `20260706-204908` classification:
- Must-fix 1, audio-only renormalized probe can over-clear confidence: accept. Added raw audio-probe evidence guard requiring at least two positive probe domains, target agreement, top score >= 0.50, and raw margin >= 0.08.
- Must-fix 2, audio-only probe behavior not validated realistically: accept. Added contested clean audio-only switch tests and noisy small-margin false-switch regression. Updated fixture probe generator to emit both expected and distractor domain scores.
- Should-fix doc path inconsistency: accept. Updated Git docs to use host-qualified `/mnt/taurus/...` paths and no longer cite `/tmp` fixture artifacts as canonical evidence.
- Should-fix unconditional per-window embedding CPU transfer: accept. `return_query_window_embeddings` is now requested only for auto glossary sessions.
- Should-fix `_retrieve_slice_groups` callsite completeness: verified with `rg -n "_retrieve_slice_groups\\(" framework/agents/omni.py`; only the two call sites and method definition exist.

Post-review patch verification:
- Local `git diff --check`: passed.
- Local py_compile: `python3 -m py_compile eval/streaming_sst/eval_auto_glossary_switch.py framework/agents/omni.py framework/agents/plugins/retrieval.py framework/agents/term_memory/topic_router.py test_auto_working_fixed_top10.py test_hybrid_window_topic_router.py`: passed.
- Taurus tmp tree refreshed after clearing root-owned pytest caches.
- Taurus targeted tests inside `sglang-omni-jaxan-rasst-index-0706`: `pytest -q test_auto_glossary.py test_auto_working_fixed_top10.py test_hybrid_window_topic_router.py test_retrieval_plugin.py test_auto_glossary_switch_eval.py --tb=short`
- Result: `28 passed in 1.43s`.
- Taurus fixture + contested probe command: `python eval/streaming_sst/eval_auto_glossary_switch.py --with-probe --max-switch-windows 2 --out-json /mnt/data2/jiaxuanluo/rasst-demo/runtime/eval/auto_glossary_switch_fixture_probe_20260707_final8.json`
- Result summary: `{"passed": true, "router_text_source": "manifest_source", "scenarios": 4, "with_probe": true}`.
- Taurus audio-only synthetic probe command: `python eval/streaming_sst/eval_auto_glossary_switch.py --with-probe --router-text-source none --max-switch-windows 3 --out-json /mnt/data2/jiaxuanluo/rasst-demo/runtime/eval/auto_glossary_switch_audio_probe_fixture_20260707_final8.json`
- Result summary: `{"passed": true, "router_text_source": "none", "scenarios": 4, "with_probe": true}`.
- Taurus source-text command: `python eval/streaming_sst/eval_auto_glossary_switch.py --acl-text /mnt/data2/jiaxuanluo/rasst_eval/acl6060_zh_smoke/source_text.txt --medicine-text /mnt/data2/jiaxuanluo/RASST/data/main_result/inputs/medicine_zh/medicine.source_text.en__medicine_404.txt --max-windows-per-domain 8 --max-switch-windows 4 --out-json /mnt/data2/jiaxuanluo/rasst-demo/runtime/eval/auto_glossary_switch_router_only_20260707_final8.json`
- Result summary: `{"passed": true, "router_text_source": "manifest_source", "scenarios": 4, "with_probe": false}`.
