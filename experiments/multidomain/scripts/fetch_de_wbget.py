    ...
    for a in range(4):
        try:
            ...
            for q,ent in d.get("entities",{}).items():
                lab=(ent.get("labels") or {}).get("de",{}).get("value")
                if lab: out[q]=lab
            return out
...
def main():
    ...
    for g in ("finance_wiki_12k","legal_wiki_12k"):
        ...
        qids=list(by_qid); de_map={}
        for i in range(0,len(qids),50):
            de_map.update(fetch(qids[i:i+50])); time.sleep(0.25)
            if (i//50)%20==0: print(f"  {g}: {i}/{len(qids)}, {len(de_map)} de", flush=True)
        filled=0
        for q,es in by_qid.items():
            de=de_map.get(q)
            if de:
                for e in es: e.setdefault("target_translations",{})["de"]=de; filled+=1
        json.dump(d, open(f"{L}/{g}.json","w"), ensure_ascii=False)
        print(f"{g}: filled {filled}/{len(d)} ({100*filled//len(d)}%)", flush=True)
