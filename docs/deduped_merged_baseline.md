...
`base_*` duplicate/conflict 统计在十个 base sources 扫描完成后冻结；top-up collisions
仍完整保留在 `duplicate_audit.json` 与 `all_*` 统计中，但不会改变 pure base union 的
审计结果。
...
| base duplicated keys with different zh targets | 594 |
| base duplicated keys with different translation maps | 901 |
| deterministic top-up selected | 11,378 |
| exact top-up output | 100,000 |

`594/901` 是 pure 10-domain union 的
`base_target_variant_conflict_term_count` / `base_translation_map_conflict_term_count`。
先前记录的 `758/1,209` 混入了 top-up occurrences，不能作为 base union 统计。
