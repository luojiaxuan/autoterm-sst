...
1. `InfiniSST` 是 RASST no-RAG condition 的历史名称：Qwen3-Omni 经过
   multi-turn、chunkwise streaming translation SFT，但关闭 MaxSim retrieval 与
   term-map injection。论文首次出现时引用 InfiniSST paper 并解释该名称。
...
- **TERM_ACC（四个 RAG rows）**：固定 2,047 个 raw tagged occurrences（NLP
  1,368；medicine 679）。2,551 个 raw annotation rows 只在 domain、block、精确
  audio span 和 normalized target variants 完全相同时合并 source aliases；不同
  target、嵌套术语和跨 domain translation 均保留。
- **InfiniSST row**：按用户指定保留 RASST PR #1 的 canonical LM=2 五-talk
  corpus reports：ACL TERM_ACC/BLEU/MT-BLEU 为 76.55/45.8268/40.7155，medicine
  为 46.28/37.8511/35.9263；All/BLEU/MT-BLEU 对两个 corpus 做 macro average。
  该 row 不再使用先前错误的 mixed-session backfill/rescoring。
- **BLEU（四个 RAG rows）**：每个 talk 的 streaming hypothesis 先经过
  RASST/StreamLAAL 的 `mwerSegmenter` 对齐到 reference sentences，再在全部 1,905
  个句子上计算 corpus sacreBLEU，不能平均 ACL/medicine 两个 BLEU，也不能对整段
  字符串直接算 BLEU。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| InfiniSST (no RAG) | 61.42 (corpus macro) | 76.55 | 46.28 | 41.84 | 38.32 | 0.00 | 0.00 |
...
- AutoTerm vs no RAG：+26.91 TERM_ACC points、+3.25 BLEU、+3.96 MT-BLEU。
...
No-RAG 数据来自 `LeiLiLab/RASST` PR #1 branch
`agent/add-masked-bleu-results` 的 `masked_terms_quality.tsv`。论文使用其中 LM=2
canonical rows，而不是错误的 mixed-session no-RAG backfill：

- ACL：TERM_ACC 0.7655、BLEU 45.8268、MT-BLEU 40.7155；
- medicine：TERM_ACC 0.4628、BLEU 37.8511、MT-BLEU 35.9263；
- table aggregate：两个五-talk corpus macro average，分别为 61.415%、41.83895、
  38.3209。

No-RAG prompt precision 与 Refs/chunk 按定义为 0。`InfiniSST` 在这里不是另一个
旧架构，而是 RASST no-RAG condition：Qwen3-Omni streaming model 经过
multi-turn/chunkwise translation SFT，运行时不使用 MaxSim 或 term-map prompt。
正文首次出现必须引用 `ouyang2025infinisst`。
...
Medicine En→Zh LM sweep。论文使用标准 system RTF：processing wall time / input
audio duration。原图的 retriever time/audio 只作为 system total 内的 component。
...
当前十-talk run 在单张 Hyper H200 上从 WebSocket connection 计时至最终完整
cursor；四个 policies 的 system RTF 分别为 Known 0.388656、AutoTerm 0.405894、
Merged-100k 0.415132、Merged-1M 0.423417。AutoTerm 的 `retrieve_s` 包含 shared
speech encoding、BGE-M3 context similarity、router observation/update 与最多四个
active-index queries；其 203.272 ms mean / audio duration = 10.588%，只称
routing/retrieval stage-time ratio，不再称 system RTF。
...
| no-RAG ACL instances | RASST `.../acl_tagged_raw_infinisst_zh/lm2/instances.log`; `9d3f95e5...05a04a` | RASST PR artifact |
| no-RAG medicine instances | Gemini `.../medicine_norag_baseline.../instances.log`; `73ebfb85...19945` | local staging；HF pending |
| no-RAG canonical score table | RASST `masked_terms_quality.tsv`; `3265c5b3...4af14` | RASST PR artifact |
...
| RASST LM system/MaxSim timing | `LeiLiLab/RASST@adc47b8`; LM `instances.log` + `rag_compute_rtf/data.tsv` | RASST source artifacts |
| AutoTerm routing+retrieval timing | AutoTerm run `3b4fbb01...32b30f`; 8,776 `retrieve_s` records | local staging；HF pending |
| ten-talk standard system RTF | [`autoterm_10talk_system_rtf_20260711.tsv`](autoterm_10talk_system_rtf_20260711.tsv) | Git canonical summary；raw runs HF pending |
...
- standard system RTF：`eval/streaming_sst/score_system_rtf.py`
- Figure 3：`eval/streaming_sst/render_budgeted_routing_timeline.py`

当前校验：materializer 与 MT-BLEU 单元测试通过；四个 RAG conditions 共用
source、reference、mask glossary 和 1,905-sentence denominator。InfiniSST row
使用上文冻结的两个 canonical five-talk corpus reports。
